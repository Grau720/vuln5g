...
export default function IADashboard() {
  ...
  // Load initial data with cache and progress
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Check cache first (5 min TTL)
        const cacheKey = 'ia_dashboard_data';
        const cacheTimeKey = 'ia_dashboard_time';
        ...
        const statsRes = await fetch("/api/v1/ia/stats");
        const statsData = await statsRes.json();
        setStats(statsData);
        
        setLoadingProgress(75);
        const topRes = await fetch("/api/v1/ia/top-risk?limit=20");
        ...
        setLoading(false);
        // Delay chart rendering to speed up initial load
        setTimeout(() => setShowCharts(true), 100);
      }
    };
    fetchData();
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (errorDiv) {
          errorDiv.textContent = 'CVE no encontrado';
      ...
      const errorDiv = document.getElementById('search-error');
      if (errorDiv) {
        errorDiv.textContent = 'Error al buscar CVE';
        setTimeout(() => errorDiv.textContent = '', 3000);
      }
    } finally {
      setSearching(false);
    ...
    return (
      <div className="global-bg min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg mb-4">Cargando análisis de IA...</p>
          ...
          <p className="text-sm text-[var(--muted)] mt-2">{loadingProgress}%</p>
        </div>
      </div>
    );
  }
    ...
    <div className="global-bg min-h-screen">
      <div className="mx-auto max-w-[1600px] p-4 md:p-6 lg:p-8">
        ...
        <header className="mb-6 flex justify-between items-center">
          ...
          <Button 
            onClick={() => {
              sessionStorage.removeItem('ia_dashboard_data');
              sessionStorage.removeItem('ia_dashboard_time');
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            ...
            <div id="search-error" className="text-red-400 text-sm mt-2 min-h-[20px]"></div>
            ...
            <CardContent style={{ height: 300 }}>
              {showCharts ? (
                ...
                <div className="flex items-center justify-center h-full">
                  <p className="text-[var(--muted)]">Cargando gráfico...</p>
                </div>
              )}
            </CardContent>
          </Card>
