...
Versión 2.2 - Optimizado para tráfico duplicado (TEE mirroring)
...
class SuricataRuleBuilder:
    ...
    def build_rce_rule(cve_data: dict, ia_data: dict, sid: int) -> str:
        ...
        if 'shell' in desc or 'metacharacter' in desc:
            # Shell metacharacters injection - buscar en payload TCP
            return (
            ...
            # RCE genérico - buscar comandos comunes
            return (
                f'alert tcp any any -> any any '
                f'(msg:"5G-RCE: Command execution attempt - {cve_id}"; '
                f'flow:established,to_server; '
                f'pcre:"/(whoami|ls|cat|pwd|id|uname|wget|curl|bash|sh)/i"; '
                f'reference:cve,{cve_id}; '
                f'classtype:web-application-attack; '
                f'priority:1; '
    ...
    def build_sqli_rule(cve_data: dict, ia_data: dict, sid: int) -> str:
        ...
        return (
            ...
            f'content:"SELECT"; nocase; within:50; '
            f'reference:cve,{cve_id}; '
            f'classtype:web-application-attack; '
            f'priority:1; '
...
class SuricataTemplates:
    """
    Sistema de generación de reglas Suricata V2.2
    Optimizado para tráfico duplicado (TEE mirroring)
    """
