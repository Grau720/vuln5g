...
def build_labels_improved(cves):
    """
    Etiquetado mejorado y más balanceado
    
    CAMBIOS:
    1. CVSS >= 7.0 (antes 7.0) con condiciones más laxas
    ...
    for cve in cves:
        ...
        tipo = cve.get("tipo", "").lower()
        is_critical_type = any(ct.lower() in tipo for ct in critical_types)
        
        # Descripción
        desc = (cve.get("descripcion_tecnica", "") + " " + 
                cve.get("descripcion_general", "")).lower()
        has_dangerous_keywords = any(kw in desc for kw in dangerous_keywords)
        
        # Componente afectado
        componente = cve.get("componente_afectado", "").lower()
        is_critical_component = any(cc in componente for cc in critical_components)
        
        # LÓGICA DE ETIQUETADO MEJORADA
        score = 0  # Contador de "puntos de explotabilidad"
        ...
        # CVSS alto + condiciones de red (peso: 40)
        if cvss_score >= 7.0:
            if is_network:
                score += 20
