...
def build_labels(cves):
    """
    Etiquetado mejorado basado en múltiples señales de explotabilidad.
    """
    y = []
    for cve in cves:
        cvss = cve.get("cvssv3", {})
        ...
        # Señales de explotabilidad
        has_exploit_refs = any(
            'exploit' in ref.lower() or 'poc' in ref.lower() 
            for ref in cve.get('referencias_mitre', [])
        )
        
        has_high_cvss = cvss_score >= 7.0
        is_network_attack = "AV:N" in vector  # Attack Vector Network
        low_complexity = "AC:L" in vector      # Attack Complexity Low
        no_privileges = "PR:N" in vector       # Privileges Required None
        no_ui = "UI:N" in vector               # User Interaction None
        
        # Lógica de etiquetado
        if has_exploit_refs:
            y.append(1)  # Definitivamente explotable
        elif has_high_cvss and is_network_attack and low_complexity and (no_privileges or no_ui):
            y.append(1)  # Muy probable explotación
        elif cvss_score >= 9.0:
            y.append(1)  # Crítico, asumimos alta explotabilidad
        else:
            y.append(0)
