...
def find_model_path():
    """Encuentra la ruta correcta de los modelos"""
    for base_path in POSSIBLE_PATHS:
        model_path = os.path.join(base_path, "exploit_model.joblib")
        feat_path = os.path.join(base_path, "featurizer.joblib")
        
        if os.path.exists(model_path) and os.path.exists(feat_path):
            logger.info(f"✅ Modelos encontrados en: {base_path}")
